...
`yano-x-composite` and `yano-x-role-workflow`. Those modules must never
...
```bash
./gradlew :products:evidence:profile:check
./gradlew -q :products:evidence:profile:roleEvidenceProfileDigest \
