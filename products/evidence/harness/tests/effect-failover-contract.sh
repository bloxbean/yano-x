...
# remains explicitly pinned; a response must never supply its own expectation.
...
for script in (source, (Path(sys.argv[1]).parent / "deployment-parity-e2e.sh").read_text(encoding="utf-8")):
    ...
    if "and .pluginApiLevel == 8" not in script:
        raise SystemExit("live demo must pin the selected API-level-8 host")
