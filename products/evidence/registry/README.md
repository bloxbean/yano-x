...
`yano-x-evidence-registry` is the ADR-013 first-party workflow component for
a product-batch inspection or compliance certificate. It lives under
`products`: it is reusable product code, while the runnable scenario
and sample business data remain under `examples`. It does not claim to
...
`yano-x-evidence-registry` ships the deterministic state machine, its
...
The registry's normal JAR is the thin compile-time artifact and declares
`yano-x-evidence-contracts` as an implementation dependency. The published
`yano-x-evidence-registry-bundle` artifact is the self-contained JVM plugin and privately
...
```bash
./gradlew :products:evidence:contracts:check \
          :products:evidence:registry:check \
          :products:evidence:client:check
```
