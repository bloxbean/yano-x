...
```bash
./gradlew :products:evidence:demo-runner:shadowJar
java -jar build/libs/yano-x-evidence-demo-runner-*-all.jar \
...
`yano-x-composite-contracts` client artifact. It commits the stock workflow's
