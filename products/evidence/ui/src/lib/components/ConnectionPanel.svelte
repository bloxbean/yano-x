...
<section class="mx-auto grid min-h-[76vh] max-w-6xl items-center gap-8 py-10 lg:grid-cols-[1.05fr_.95fr]">
  <div class="px-2 lg:px-8">
    ...
    <h1 class="mt-5 max-w-3xl text-4xl font-semibold leading-[1.06] tracking-[-.045em] text-white sm:text-6xl">
      Release evidence<br /><span class="text-mint-400">with independent approvals.</span>
    </h1>
    <p class="mt-6 max-w-xl text-base leading-7 text-[#8ea8a0]">
      The Evidence Desk is the workbench for role-gated release on a Yano app chain. Issuers propose,
      auditors from different organizations decide with keys that never leave their browser, and the
      approved release is applied once. Every record the desk shows is read back from the chain's
      authenticated state with its proof.
    </p>
    <div class="mt-8 grid max-w-xl grid-cols-3 gap-3">
      ...
      <div class="panel p-4"><div class="eyebrow">02</div><div class="mt-2 text-sm">Load an actor key</div></div>
      <div class="panel p-4"><div class="eyebrow">03</div><div class="mt-2 text-sm">Propose, decide, release</div></div>
    </div>
  </div>

  <div class="glass p-6 sm:p-8">
    <div class="eyebrow">Connection</div>
    <h2 class="mt-2 text-xl font-semibold tracking-tight">Open a Yano installation</h2>
    ...
    <div class="mt-6 border-t border-white/[.07] pt-4 text-[.72rem] leading-5 text-[#71887f]">
      The desk lists only chains whose composite carries the domain-actors and role-approvals
      components. Actor seeds and API keys stay in this browser tab and are never sent to the node.
    </div>
  </div>
</section>
