...
const MAX_TEXT = 256;
export const RUNTIME_CONFIG_FILE = 'feed-ui-config.json';
...
type PublicEnvironment = {
  DEV?: boolean;
  VITE_FEED_SERVICE_URL?: string;
  VITE_FEED_GATEWAY_URL?: string;
};

export function resolveConnectionDefaults(environment: PublicEnvironment): ConnectionDefaults {
  return {
    serviceUrl: environment.VITE_FEED_SERVICE_URL?.trim() || 'http://localhost:8680',
    gatewayUrl: environment.VITE_FEED_GATEWAY_URL?.trim() || 'http://localhost:8690'
  };
}
...
export function parseRuntimeConfig(value: unknown): RuntimeConfig {
  if (!isRecord(value) || value.schemaVersion !== 1 || value.productId !== 'attestation-feed') {
    throw new Error('The attestation feed runtime configuration has an unsupported identity');
  }
  ...
  return {
    schemaVersion: 1, productId: 'attestation-feed', serviceUrl, gatewayUrl,
    allowServiceOverride: value.allowServiceOverride
  };
}
