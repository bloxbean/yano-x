...
const MAX_TEXT = 256;
export const RUNTIME_CONFIG_FILE = 'attest-ui-config.json';
...
export function parseRuntimeConfig(value: unknown): RuntimeConfig {
  if (!isRecord(value) || value.schemaVersion !== 1 || value.productId !== 'attest') {
    throw new Error('The Attest UI runtime configuration has an unsupported identity');
  }
  if (!Array.isArray(value.endpoints) || value.endpoints.length > MAX_ENDPOINTS) {
    throw new Error('The Attest UI runtime configuration has too many endpoints');
  }
