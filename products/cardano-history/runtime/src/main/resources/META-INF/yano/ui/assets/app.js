(() => {
  ...
  async function loadOverview(){
    ...
    const available=new Set(capabilities);document.querySelectorAll('[data-capability]').forEach(node=>{node.hidden=!available.has(node.dataset.capability);});
  ...
  function selectProof(result){selected=result;currentBundle=null;byId('export').disabled=true;const proof=result?.proof;const primary=proof?.physicalKey||proof?.factPhysicalKey;byId('generate').disabled=!primary;byId('proof-status').textContent=primary?'MPF proof coordinates ready. Open Proof lab to verify locally.':'This dataset uses an authenticated snapshot or has no primary proof coordinate; export its full bundle with the CLI.';}
  function envelope(value){return{...value,chainId:String(value.chainId),committedHeight:Number(value.committedHeight),key:String(value.key).toLowerCase(),stateRoot:String(value.stateRoot).toLowerCase(),profile:String(value.profile),presence:value.presence,valueHex:value.valueHex?String(value.valueHex).toLowerCase():undefined,proofWireHex:String(value.proofWireHex).toLowerCase()};}
  function verify(bundle){const verdict=globalThis.CardanoHistoryMpf.verifyBundle(bundle);byId('proof-status').textContent=verdict.valid?'PASS — MPF wire, key/value/height/root/profile and history bindings verified locally. Cardano anchor not independently verified.':'FAIL — local proof or history binding mismatch.';byId('proof-result').textContent=pretty(verdict);byId('bundle').value=pretty(bundle);currentBundle=bundle;byId('export').disabled=false;return verdict;}
  byId('generate').addEventListener('click',async()=>{const proof=selected?.proof;const factKey=proof?.physicalKey||proof?.factPhysicalKey;if(!factKey)return;byId('proof-result').textContent='Loading proof bytes…';try{const fact=envelope(await call('app-chain.proof',{keyHex:factKey,height:selected.committedHeight}));let completeness=null;if(proof.completenessPhysicalKey)completeness=envelope(await call('app-chain.proof',{keyHex:proof.completenessPhysicalKey,height:selected.committedHeight}));const bundle={schema:'cardano-history-browser-proof-v1',chainId:session.chainId,committedHeight:selected.committedHeight,stateRoot:String(selected.stateRoot).toLowerCase(),factKey:String(factKey).toLowerCase(),requiresCompleteness:Boolean(proof.completenessPhysicalKey),history:selected,fact,completeness};verify(bundle);}catch(error){byId('proof-status').textContent='FAIL — '+error.message;showError(error);}});
  ...
  byId('export').addEventListener('click',async()=>{if(!currentBundle)return;try{await navigator.clipboard.writeText(pretty(currentBundle));byId('proof-status').textContent='Bundle copied. Local result remains unchanged.';}catch(error){byId('bundle').select();showError('Clipboard unavailable; copy the selected bundle text.');}});
