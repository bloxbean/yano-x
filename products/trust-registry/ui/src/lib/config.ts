...
const MAX_TEXT = 256;
export const RUNTIME_CONFIG_FILE = 'trust-registry-ui-config.json';
...
export function parseRuntimeConfig(value: unknown): RuntimeConfig {
  if (!isRecord(value) || value.schemaVersion !== 1 || value.productId !== 'trust-registry') {
    throw new Error('The Trust Registry runtime configuration has an unsupported identity');
  }
  if (!Array.isArray(value.endpoints) || value.endpoints.length > MAX_ENDPOINTS) {
    throw new Error('The Trust Registry runtime configuration has too many endpoints');
  }
...
/** Base URL of the standards service; a path is allowed, credentials and queries are not. */
export function normalizeServiceUrl(value: string, origin = globalThis.location?.origin): string {
  const trimmed = value.trim().replace(/\/+$/, '');
  if (!trimmed) throw new Error('Enter the URL of a yano-trust service');
  const parsed = new URL(trimmed, origin);
  if (parsed.username || parsed.password || parsed.search || parsed.hash) {
