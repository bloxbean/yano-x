...
<section class="mx-auto grid min-h-[76vh] max-w-6xl items-center gap-8 py-10 lg:grid-cols-[1.05fr_.95fr]">
  <div class="px-2 lg:px-8">
    ...
    <h1 class="mt-5 max-w-3xl text-4xl font-semibold leading-[1.06] tracking-[-.045em] text-white sm:text-6xl">
      Trust and status,<br /><span class="text-mint-400">answered with proofs.</span>
    </h1>
    <p class="mt-6 max-w-xl text-base leading-7 text-[#8ea8a0]">
      The Trust Registry console reads a governed authenticated-map chain that carries the
      registry profile: subjects, credential status, published status lists, and issuers. Every
      answer is a state proof bound to a certified block, with who wrote the entry and under which
      policy. It never signs and never writes.
    </p>
    <div class="mt-8 grid max-w-xl grid-cols-3 gap-3">
      ...
      <div class="panel p-4"><div class="eyebrow">02</div><div class="mt-2 text-sm">Look up an entry</div></div>
      <div class="panel p-4"><div class="eyebrow">03</div><div class="mt-2 text-sm">Export and verify offline</div></div>
    </div>
  </div>
  ...
  <div class="glass p-6 sm:p-8">
    ...
    <div class="mt-6 border-t border-white/[.07] pt-4 text-[.72rem] leading-5 text-[#71887f]">
      The console lists governed authenticated-map chains and marks those whose genesis declares
      the registry profile. API keys stay in this browser tab and are never stored.
    </div>
  </div>
</section>
