...
```bash
./gradlew :state-machines:zk:shadowJar
# state-machines/zk/build/libs/
#   yano-x-zk-<version>-bundle.jar
```
...
a catalog-v1 deployment unit. The Yano X ZK extension is JVM-only.
...
```bash
./gradlew :state-machines:zk:test
```
