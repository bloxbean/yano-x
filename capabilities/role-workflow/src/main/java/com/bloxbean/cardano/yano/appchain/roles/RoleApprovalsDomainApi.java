...
public final class RoleApprovalsDomainApi implements DomainApi {
    ...
    public RoleApprovalsDomainApi(DomainApiContext context) {
        this(context, RoleApprovalsStateMachineProvider.ID);
    }

    public RoleApprovalsDomainApi(DomainApiContext context, String expectedStateMachineId) {
        this.context = Objects.requireNonNull(context, "context");
        this.expectedStateMachineId = Objects.requireNonNull(
                expectedStateMachineId, "expectedStateMachineId");
    }

    public static boolean supports(String routeId) {
    ...
    public DomainApiResponse handle(DomainApiRequest request) {
        ...
        if (!STATS.equals(request.routeId())) {
            ...
            try {
                id = RoleWorkflowIdentifiers.id(id, "id");
        ...
        if ((PROPOSAL.equals(request.routeId()) || STATS.equals(request.routeId()))
    ...
    private AppQueryResult query(String chain, String path, String params) {
        ...
        if (!chain.equals(result.chainId())
                || !expectedStateMachineId.equals(result.stateMachineId())) {
            throw new DomainApiException(DomainApiException.Code.FAILED,
    ...
    private static String actor(ActorRecordV1 value) {
        String roles = value.roles().stream().map(RoleApprovalsDomainApi::string)
    ...
    private static String policy(ApprovalPolicyV1 value) {
        String proposers = value.proposerRoles().stream().map(RoleApprovalsDomainApi::string)
                .collect(java.util.stream.Collectors.joining(",", "[", "]"));
