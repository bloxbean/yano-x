...
public final class RoleWorkflowDomainApi implements DomainApi {
    ...
    RoleWorkflowDomainApi(DomainApiContext context) {
    ...
    private static byte[] proofKey(String route, String id, long revision, byte[] payload) {
        byte[] local = switch (route) {
            ...
            case EVIDENCE_APPROVAL -> RoleWorkflowKeys.evidenceApproval(id, revision);
    ...
    private static String actor(ActorRecordV1 value) {
        String roles = value.roles().stream().map(RoleWorkflowDomainApi::string)
    ...
    private static String policy(ApprovalPolicyV1 value) {
        String proposers = value.proposerRoles().stream().map(RoleWorkflowDomainApi::string)
                .collect(java.util.stream.Collectors.joining(",", "[", "]"));
