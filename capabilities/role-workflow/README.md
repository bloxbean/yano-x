...
[`yano-x-evidence-profile`](../../products/evidence/profile/README.md).
...
from `yano-x-role-workflow-contracts` with their KMS, HSM, vault, or client
signer.

## Build

```bash
./gradlew :capabilities:role-workflow-contracts:check \
          :capabilities:role-workflow:check
```
