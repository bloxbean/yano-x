...
- `yano-x-eutxo-ledger` — optional state-machine plugin and transition
  engine.
...
- `yano-x-eutxo-indexer-core` — storage-neutral projection, reader,
  checkpoint, health, lineage, and store-conformance contracts.
- `yano-x-eutxo-indexer-jdbc` — Flyway-managed SQLite journal and relational
  query projection.
...
into the thin plugin bundle. The EUTxO extension is deployed through the Yano X
JVM distribution; native runtime compatibility is not claimed.
...
```bash
./gradlew :ledgers:eutxo:testkit:runNoFundsDemo
```
