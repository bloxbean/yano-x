...
configuration are unchanged from the pre-release `yano-x-kafka-sink`
...
# connectors/kafka/build/libs/
#   yano-x-kafka-<version>-bundle.jar
```
...
The ordinary `yano-x-kafka-<version>.jar` remains a compile-time library.
Runtime deployment uses the JVM bundle; Yano X does not publish native
connector variants.
...
```bash
./gradlew :connectors:kafka:check

# Optional real-broker acknowledgement test
./gradlew :connectors:kafka:test \
...
products/evidence/harness/tests/connector-fault-matrix.sh
...
connectors/kafka/src/test/scripts/kafka-secure-integration.sh
```
