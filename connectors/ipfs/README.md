...
```bash
./gradlew :connectors:ipfs:shadowJar
# connectors/ipfs/build/libs/
#   yano-x-ipfs-<version>-bundle.jar
```
...
The ordinary `yano-x-ipfs-<version>.jar` stays thin for compile-time use.
Runtime deployment uses the JVM bundle; Yano X does not publish a native IPFS
connector.
...
./gradlew \
  ...
  :connectors:ipfs:test --tests '*KuboIpfsPinClientRealIntegrationTest'
