...
| Yano pre14 (merge of `main` after PR #4, 2026-09-06) | `main` now needs the unreleased Yano pre14, built as ADR-045 and ADR-047 §15 record. The desk reads blocks and records through JSON endpoints and never parses block CBOR, so no change was needed; the golden test, unit tests, and distribution verification were re-run against pre14. The live and browser runs above were on the pre13 showcase and were not repeated on pre14. |
