...
import { DOCSITE_ROOT } from './repo-sources.mjs';
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (const [, href] of html.matchAll(hrefPattern)) {
      ...
      if (hash === -1) continue;
      const clean = href.slice(0, hash).split('?')[0];
      const fragment = href.slice(hash + 1);
      if (!clean || !fragment) continue;
      // Some hashes are application state rather than an element anchor — the
      // App-Chain Studio encodes a whole blueprint as `#recipe=…&members=3&…`.
      if (clean.startsWith('/studio/') || /[=&]/.test(fragment)) continue;
